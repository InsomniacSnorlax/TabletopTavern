...
namespace TJ
{
    public class GearManager : MonoBehaviour
    {
        ...
        public List<UnitStatBonus> GetGearStatBonus(UnitStat _unitStat, UnitName _requestingUnit)
        {
            ...
            {
                List<UnitAttributeBonus> unitAttributeBonuses = GetGearAttributeBonus(_squadStats.unitName);
            ...
            if(activeGearItems.Contains(GearID.Longbows)) {
                if(_unitStat == UnitStat.Range && TabletopTavernData.Instance.GetUnitTypeFromUnitName(_requestingUnit) == UnitType.Ranged) {
                    ...
                    unitStatBonuses.Add(new UnitStatBonus(_unitStat, gearNameLocalized, GearData.Longbows.GearModifierValue));
                }
            }
            if(activeGearItems.Contains(GearID.Glaives)) {
                if(_unitStat == UnitStat.WeaponStrength && squadStats.SquadAttributes.AntiLarge) {
                    ...
                    unitStatBonuses.Add(new UnitStatBonus(_unitStat, gearNameLocalized, GearData.Glaives.GearModifierValue));
                }
            }
            if(activeGearItems.Contains(GearID.ConscriptionOrders)) {
                ...
                if(_unitStat == UnitStat.MeleeAttack && squadStats.RarityTier == UnitRarity.Common) {
                    unitStatBonuses.Add(new UnitStatBonus(_unitStat, gearNameLocalized, GearData.ConscriptionOrders.GearModifierValue));
                }
            ...
            if(activeGearItems.Contains(GearID.JoustingLances) && squadStats.unitSize == UnitSize.Cavalry) {
                if(_unitStat == UnitStat.WeaponStrength) {
                    ...
                    unitStatBonuses.Add(new UnitStatBonus(_unitStat, gearNameLocalized, GearData.JoustingLances.GearModifierValue));
                }
            }
            if(activeGearItems.Contains(GearID.GnomishArmorers)) {
                if(_unitStat == UnitStat.Armor && squadStats.RarityTier == UnitRarity.Uncommon) {
                    ...
                    unitStatBonuses.Add(new UnitStatBonus(_unitStat, gearNameLocalized, GearData.GnomishArmorers.GearModifierValue));
                }
            }
            if(activeGearItems.Contains(GearID.WellHonedAxes)) {
                if(_unitStat == UnitStat.MeleeAttack && squadStats.SquadAttributes.ArmorPiercing) {
                    ...
                    unitStatBonuses.Add(new UnitStatBonus(_unitStat, gearNameLocalized, GearData.WellHonedAxes.GearModifierValue));
                }
            }
            ...
            if(activeGearItems.Contains(GearID.RingoftheElvenKing)) {
                if(_unitStat == UnitStat.MissileStrength && squadStats.unitType == UnitType.Ranged) {
                    ...
                    unitStatBonuses.Add(new UnitStatBonus(_unitStat, gearNameLocalized, GearData.RingoftheElvenKing.GearModifierValue));
                }
            }
            if(activeGearItems.Contains(GearID.QuantitativeEasingPolicy)) {
                if(_unitStat == UnitStat.Leadership) {
                    ...
                    unitStatBonuses.Add(new UnitStatBonus(_unitStat, gearNameLocalized, GearData.QuantitativeEasingPolicy.GearModifierValue));
                }
            }
            if(activeGearItems.Contains(GearID.Shungite)) {
                if(_unitStat == UnitStat.MeleeAttack && squadStats.RarityTier == UnitRarity.Uncommon) {
                    ...
                    unitStatBonuses.Add(new UnitStatBonus(_unitStat, gearNameLocalized, GearData.Shungite.GearModifierValue));
                }
            }

            return unitStatBonuses;
        }
        public List<UnitAttributeBonus> GetGearAttributeBonus(UnitName _requestingUnit)
        {
